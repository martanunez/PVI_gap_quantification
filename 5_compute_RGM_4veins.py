...
"""
    Compute Relative Gap Measure considering the 4 Pulmonary Veins independently.
    ...
    Usage: python 5_compute_RGM_4veins.py --meshfile data/mesh.vtk
"""
...
if platform == 'linux' or platform == 'linux2':
    ...
    workbook = xlsxwriter.Workbook(fileroot + '/gap_quantification_results/RGM_results_4veins.xlsx')
...
worksheet.write(0, 1, 'RSPV', format)
worksheet.write(0, 2, 'RIPV', format)
worksheet.write(0, 3, 'LIPV', format)
worksheet.write(0, 4, 'LSPV', format)
worksheet.write(1, 0, 'Gap length', format)
worksheet.write(2, 0, 'RGM', format)
worksheet.write(3, 0, '# gaps', format)
worksheet.set_column('A:E', 15)  # Widen columns.
...
pv_labels = ['rspv', 'ripv', 'lipv', 'lspv']
for vein_i in range(4):
    pv_label = pv_labels[vein_i]
    vein = pointthreshold(m3d, pv_label, float(1), float(1))
    outfile_vein = string_path + filename_base + '_vein_' + pv_label + '.vtk'
    writevtk(vein, outfile_vein)

    # open the mesh in the carina but not in the other limit where up_down_label changes its value
    [cut_vein, ids_lim1, ids_lim2] = cut_mesh_carina(vein, 'up_down_label', 0, 1, 'carinas', 0)
    ...
    outfile_open = string_path + filename_base + '_vein_' + pv_label + '_final.vtk'
...
#         writevtk(edges, outfile2)
    ...
    # print(pv_labels[vein_i], 'has', nblobs, 'blobs')
    ...
    filename_mesh_art_blobs = string_path + filename_base + '_vein_' + pv_label + '_with_all_blobs.vtk'
    writevtk(mesh, filename_mesh_art_blobs)

    # first and last paths must be calculated at the same time since we must enforce a closed path (where one path finishes the next one starts)
    dual_points = find_dual_points_limits(mesh, blob_0, blob_n)  # In each column two dual points (row 0 has points from blob0, row 1 has points from blobn)
    dual_points = dual_points.astype(dtype=int)
    blob_ids = blob_ids.astype(dtype=int)
    #print 'Dual points: ', dual_points

    if dual_points.size == 0:     # Do this before DT calculation to avoid memory crash
    ...
    for bb in range(nblobs):
        ...
        if blob_j.size == 0:
            print('Blob with index (new): ', bb, ' disappeared')
            # I have to calculate distance transform for this blob. Use old function.
            ...
            # update with correct distance transform of the missing blob
            distance_transforms[:, bb] = distance_transforms_aux[:, bb-1]    # aux has 2 less columns. bb cannot be 0. new_blob_array == 0 means artificial blob0 and that one cannot 'disappear'
            # update all the distances to the points of the missing blob. Careful with the sizes
    ...
    if (total_gap_length!=0):
        ...
        for ind in range(len(path)-1):   # Careful! limit2 is not within vertex_names
...
# merge all polydatas
merge_polydatas_gaps_blobs_4veins(string_path)
# count gaps, write info
nb_gaps = count_gaps_4veins(string_path)
worksheet.write(3, 1, nb_gaps[0], format_int)
worksheet.write(3, 2, nb_gaps[1], format_int)
worksheet.write(3, 3, nb_gaps[2], format_int)
worksheet.write(3, 4, nb_gaps[3], format_int)
workbook.close()
...
if platform == 'linux' or platform == 'linux2':
    ...
    command = 'del '
for i in range(len(pv_labels)):    # pv_labels = ['rspv', 'ripv', 'lipv', 'lspv']
     os.system(command + string_path + pv_labels[i] + '_gap_*.vtk')
     os.system(command + string_path + pv_labels[i] + '_dist_within_*.vtk')
     os.system(command + string_path + 'mesh_vein_' + pv_labels[i] + '.vtk')
     os.system(command + string_path + 'mesh_vein_' + pv_labels[i] + '_DT.vtk')
     os.system(command + string_path + 'mesh_vein_' + pv_labels[i] + '_final.vtk')
     os.system(command + string_path + 'mesh_vein_' + pv_labels[i] + '_with_all_blobs.vtk')
